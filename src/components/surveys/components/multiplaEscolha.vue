    ...
    <div>
        <div v-if="displayON == false" class="w-full">
            <draggable :list="value"  animation="200"  handle=".my-handle" >
                <div v-for="(op, index) in value" :key="index" class="flex w-full" >
                    ...
                    <button 
                        class="ml-1 flex items-center justify-center h-4 w-4 rounded-full  focus:ring-inset  hover:ring-gray-400 focus:outline-none transform hover:scale-110" @click="removeOption(index)" 
                    >	
        ...
        <div v-if="displayON == true" class="mt-5">
            <div v-for="(op, index) in value" :key="index" class="flex items-center justify-start w-full mt-2" >
                <input class="" type="radio" name="radiog" value="op.count">
                <label for="op.count" class="text-center text-md ml-2"> 
                       {{ op.text }}             
                </label>
            </div>	   
        </div>
